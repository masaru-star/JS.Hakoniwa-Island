...
const MONSTER_TYPES = {
  1: {
    ...
    moneyPerTurn: 0,
    defeatMoney: 0,
    condition: (pop, currentTurn) => pop >= 100000,
  },
  2: {
    name: "怪獣ヴォルカガロス",
    minHP: 2,
    ...
    moneyPerTurn: 0,
    defeatMoney: 0,
    condition: (pop, currentTurn) => pop >= 120000,
  },
  3: {
    name: "怪獣アエロガロス",
    minHP: 3,
  ...
  4: {
    ...
    moneyPerTurn: 0,
    defeatMoney: 0,
    condition: (pop, currentTurn) => pop >= 150000,
  },
  5: {
    name: "怪獣アクアガロス",
    minHP: 3,
    ...
    moneyPerTurn: 0,
    defeatMoney: 0,
    condition: (pop, currentTurn) => pop >= 200000,
  },
  6: {
    name: "怪獣シルバガロス",
    minHP: 2,
    ...
    moneyPerTurn: 5000,
    defeatMoney: 5000000,
    condition: (pop, currentTurn) => pop >= 100000 && currentTurn >= 3000,
  },
  7: {
    name: "怪獣ゴルドガロス",
    minHP: 2,
    ...
    ability: null,
    moneyPerTurn: 50000,
    defeatMoney: 50000000,
    condition: (pop, currentTurn) => pop >= 130000 && currentTurn >= 3000,
  },
  8: {
    name: "怪獣プラチガロス",
    minHP: 2,
    ...
    ability: null,
    moneyPerTurn: 500000,
    defeatMoney: 250000000,
    condition: (pop, currentTurn) => pop >= 150000 && currentTurn >= 3000,
  },
  9: {
    name: "怪獣キングガロス",
    minHP: 100,
...
async function importSigningPrivateKey(keyPair = islandSigningKeyPair) {
  if (!keyPair || !keyPair.privateKey || !isCryptoSigningAvailable())
    return null;
...
async function signWarshipName(ship) {
  ...
  const signature = await crypto.subtle.sign(
    ...
    privateKey,
    new TextEncoder().encode(ship.name),
  );
  ship.nameSignature = arrayBufferToBase64(signature);
  ship.signaturePublicKey = JSON.parse(
    JSON.stringify(islandSigningKeyPair.publicKey),
  );
}
...
async function ensureIslandSigningKeyPair(options = {}) {
  if (
    islandSigningKeyPair &&
    islandSigningKeyPair.publicKey &&
    islandSigningKeyPair.privateKey
  ) {
  ...
  if (islandSigningKeyPair) {
    ...
    if (!options.silent)
      logAction("自島のP-256署名鍵ペアを作成し、保有軍艦に署名しました。");
    updatePublicKeyDisplay();
  }
  return islandSigningKeyPair;
}
...
async function hasValidWarshipNameSignature(ship) {
  ...
  try {
    ...
    return crypto.subtle.verify(
      { name: "ECDSA", hash: "SHA-256" },
      publicKey,
      base64ToArrayBuffer(ship.nameSignature),
      new TextEncoder().encode(ship.name),
    );
  } catch (e) {
...
async function ensureAndVerifyOwnWarship(ship) {
  ...
  if (
    !ownPublicKey ||
    JSON.stringify(ship.signaturePublicKey) !== JSON.stringify(ownPublicKey)
  )
    return false;
...
function updatePublicKeyDisplay() {
  ...
  if (!el) return;
  el.value =
    islandSigningKeyPair && islandSigningKeyPair.publicKey
    ...
    tiers: [
      {
        level: 1,
        id: "01_1_precise",
        icon: "src/svg/01_1_precise.svg",
        condition: (s) => s.maxHitStreak >= 3,
      },
      {
        level: 2,
        id: "01_2_precise",
        icon: "src/svg/01_2_precise.svg",
        condition: (s) => s.maxHitStreak >= 6,
      },
      {
        level: 3,
        id: "01_3_precise",
        icon: "src/svg/01_3_precise.svg",
        condition: (s) => s.maxHitStreak >= 12,
      },
    ],
  },
  shield: {
    ...
    tiers: [
      {
        level: 1,
        id: "02_1_shield",
        icon: "src/svg/02_1_shield.svg",
        condition: (s) => s.damageTaken > 5,
      },
      {
        level: 2,
        id: "02_2_shield",
        icon: "src/svg/02_2_shield.svg",
        condition: (s) => s.damageTaken > 10,
      },
      {
        level: 3,
        id: "02_3_shield",
        icon: "src/svg/02_3_shield.svg",
        condition: (s) => s.damageTaken > 30,
      },
    ],
  },
  shot: {
    ...
    tiers: [
      {
        level: 1,
        id: "03_1_shot",
        icon: "src/svg/03_1_shot.svg",
        condition: (s) => s.sunkWarships >= 1,
      },
    ],
  },
  avoidance: {
    ...
    tiers: [
      {
        level: 1,
        id: "04_1_avoidance",
        icon: "src/svg/04_1_avoidance.svg",
        condition: (s) => s.hp1HoldTurns >= 2,
      },
    ],
  },
  avoidance: {
...
function ensureWarshipFields(ship) {
  ...
  if (!ship.medalsEarned || typeof ship.medalsEarned !== "object")
    ship.medalsEarned = {};
...
function renderDockedWarshipCards() {
  ...
  if (!dockedWarships.length) {
    container.innerHTML =
      '<h4>船渠</h4><p class="dock-empty">収納中の軍艦はありません。</p>';
    return;
  }
  const cards = dockedWarships
    .map((ship, index) => {
      ensureWarshipFields(ship);
...
function getWarshipMedalIconsHtml(ship) {
  ...
  Object.keys(MEDAL_DEFS).forEach((key) => {
    ...
    if (tier) {
      icons.push(
        `<img class="warship-medal" src="${tier.icon}" alt="${MEDAL_DEFS[key].label} Lv${level}" title="${MEDAL_DEFS[key].label} Lv${level}">`,
      );
    }
  });
  return icons.join("");
...
function maybeLogResidentWhisper(originMsg) {
  ...
  const whisper =
    RESIDENT_WHISPERS[Math.floor(Math.random() * RESIDENT_WHISPERS.length)];
  logAction(`住民の呟き ${whisper}`, { subtle: true, skipWhisper: true });
}
function initWarshipTurnStats() {
  warshipTurnStats = {};
  warships
    .filter((ship) => !isWarshipDocked(ship))
    .forEach((ship) => {
      ensureWarshipFields(ship);
      warshipTurnStats[getWarshipKey(ship)] = {
        maxHitStreak: 0,
        currentHitStreak: 0,
        damageTaken: 0,
        sunkWarships: 0,
...
function registerWarshipHit(ship) {
  ...
  if (!warshipTurnStats[key])
    warshipTurnStats[key] = {
      maxHitStreak: 0,
      currentHitStreak: 0,
      damageTaken: 0,
      sunkWarships: 0,
  ...
  warshipTurnStats[key].maxHitStreak = Math.max(
    warshipTurnStats[key].maxHitStreak,
    warshipTurnStats[key].currentHitStreak,
  );
}
...
function evaluateWarshipMedals() {
  ...
  warships.forEach((ship) => {
    ensureWarshipFields(ship);
    const key = getWarshipKey(ship);
    const stats = warshipTurnStats[key] || {
      maxHitStreak: 0,
      damageTaken: 0,
      sunkWarships: 0,
    };
    const progress = store[key] || { hp1HoldTurns: 0 };
    progress.hp1HoldTurns =
      ship.currentDurability === 1 ? (progress.hp1HoldTurns || 0) + 1 : 0;
    Object.keys(MEDAL_DEFS).forEach((medalKey) => {
      ...
      MEDAL_DEFS[medalKey].tiers.forEach((tier) => {
        if (
          tier.condition({ ...stats, hp1HoldTurns: progress.hp1HoldTurns }) &&
          tier.level > newLevel
        ) {
          newLevel = tier.level;
        }
      ...
      if (newLevel > currentLevel) {
        ...
        logAction(
          `軍艦 ${getWarshipDisplayName(ship)} が勲章「${MEDAL_DEFS[medalKey].label} Lv${newLevel}」を獲得しました！`,
        );
      }
    });
    store[key] = progress;
...
function checkAbnormalityOnDamage(warship, damage) {
  ...
  if (warship.currentAmmo >= 1000 && Math.random() < 0.3) {
    newAbnormality = "ammoFire";
  }
  if (newAbnormality === null && Math.random() < 0.1) {
    newAbnormality = "fire";
  }
...
function getActionName(action, x, y, extraData) {
  ...
  const actionNames = {
    ...
    spreadBombard: "拡散弾砲撃",
    ppBombard: "PP弾砲撃",
    randomBombard: "ランダム弾砲撃",
    concentratedFire: "集中砲撃",
    selfDestructMilitaryFacility: "軍事施設自爆",
    goToOtherIsland: "他の島に行く",
    returnToMyIsland: "自島に戻る",
    buildWarship: "軍艦建造",
    refuelWarship: "燃料補給",
    resupplyWarshipAmmo: "弾薬補給",
    repairWarship: "軍艦修理",
    enhanceWarship: "軍艦増強",
    ...
    emergencyReturn: "緊急帰還",
    returnWarshipFromDock: "海域へ復帰",
    setWarshipNickname: "二つ名指定",
    showWarshipDetails: "詳細情報提示",
    convertAchievementToExp: "実績pt変換",
    remodelWarshipWeapon: "武器換装",
    buildMonument: "石碑建設",
  ...
  if (action === "exportFood" && extraData && extraData.amount) {
    ...
    (action === "bombard" ||
      action === "spreadBombard" ||
      action === "ppBombard" ||
      action === "randomBombard") &&
    extraData &&
    extraData.count
  ) {
    ...
    (action === "dispatchWarship" ||
      action === "requestWarshipReturn" ||
      action === "moveWarshipToDock" ||
      action === "returnWarshipFromDock") &&
    extraData &&
    extraData.name
  ) {
    name += ` (${extraData.name})`;
  } else if (
    action === "setWarshipNickname" &&
    extraData &&
    extraData.nickname
  ) {
    name += ` (${extraData.nickname})`;
  } else if (
    action === "convertAchievementToExp" &&
    extraData &&
    extraData.amount
  ) {
    name += ` (${extraData.amount}Pt)`;
  } else if (
    action === "remodelWarshipWeapon" &&
    extraData &&
    extraData.weaponType
  ) {
...
function getSelectedTileWarship() {
  ...
  return (
    warships.find((ship) => ship.x === selectedX && ship.y === selectedY) ||
    null
  );
}
function logSelectedWarshipDetails() {
  ...
  logAction(
    `軍艦「${getWarshipDisplayName(warship)}」の詳細情報: 主砲 ${warship.mainGun} / 魚雷 ${warship.torpedo} / 対空砲 ${warship.antiAir} / 偵察機 ${warship.reconnaissance} / 射撃精度 ${warship.accuracyImprovement}`,
  );
  return true;
}
...
function applyAutomaticInputValues(action) {
  if (
    isSessionSettingEnabled("settingAutoExportFoodMax") &&
    action === "exportFood"
  ) {
    document.getElementById("exportAmount").value = getMaxExportFoodAmount();
  }
  if (
    isSessionSettingEnabled("settingAutoBombardCountToGuns") &&
    (action === "bombard" ||
      action === "spreadBombard" ||
      action === "ppBombard" ||
      action === "randomBombard")
  ...
  if (isSessionSettingEnabled("settingAutoSupplyMax")) {
    ...
    if (action === "refuelWarship") {
      document.getElementById("refuelAmount").value =
        getMaxRefuelAmount(warship);
    } else if (action === "resupplyWarshipAmmo") {
      document.getElementById("resupplyAmmoAmount").value =
        getMaxResupplyAmmoAmount(warship);
    }
  }
}
function applySessionSettings() {
  setElementDisplayById(
    "clearTileSelectionBtn",
    isSessionSettingEnabled("settingShowClearTileSelection"),
    "inline-block",
  );
  setElementDisplayById(
    "keepOptionSettingControl",
    isSessionSettingEnabled("settingShowKeepOptionSelected"),
    "inline",
  );
  setElementDisplayById(
    "fundingTrackingControls",
    isSessionSettingEnabled("settingShowFundingTrackingControls"),
    "inline",
  );
  ...
  if (action === "warshipTool" && warshipSubSelect)
...
function getEconomicCrisisRiskInfo() {
  ...
  if (currentTotalMoney < threshold) {
    return {
      probability: 0,
      shortage: threshold - currentTotalMoney,
      crisisActive: false,
    };
  }
...
function updateLogStatusLines() {
  ...
  line1.style.display = isSessionSettingEnabled("settingShowEconomicCrisisRate")
    ? ""
    : "none";
  line2.style.display = isSessionSettingEnabled("settingEnablePlanTracking")
    ? ""
    : "none";
...
function setFundingFailureTrackingFromTask(task) {
  ...
  if (!task) return;
  const pinTracking = document.getElementById(
    "pinFundingFailureTracking",
  )?.checked;
...
window.cancelAction = function (index) {
  if (index >= 0 && index < actionQueue.length) {
    ...
    const { name, coord } = getActionName(
...
function checkAndCompleteMission(
  missionId,
  pt,
  foodReward,
  moneyReward,
  ...
  if (!tutorialMissions[missionId] && checkFunc()) {
    ...
    money += moneyReward;
    logAction(
      `チュートリアルミッション${missionId}「${logMessage}」を達成し、${pt}Pt、${moneyReward}G、${foodReward}食料を獲得しました！`,
    );
    updateStatus();
    saveMyIslandState();
    return true;
...
function handleMonsterDefeat(monster, customMessage) {
  ...
  if (monsterType.defeatMoney && monsterType.defeatMoney > 0) {
    ...
    logAction(
      `${monsterType.name}の討伐報奨金として${monsterType.defeatMoney}Gを獲得しました！`,
    );
  }
  if (monster.typeId === KING_MONSTER_TYPE_ID) {
    ...
    if (kingDefeatPt > 0) {
      ...
      logAction(
        `${monsterType.name}討伐により ${kingDefeatPt} 実績ptを獲得しました！`,
      );
    }
  }
}
...
function applyKingMonsterTileDestruction(x, y, actionLogPrefix) {
  ...
  if (targetWarship) {
    ...
    logAction(
      `${actionLogPrefix} 軍艦 ${targetWarship.name} に10ダメージと浸水を与えました。`,
    );
    if (targetWarship.currentDurability <= 0) {
      ...
      logAction(
        `軍艦 ${targetWarship.name} はキングガロスの攻撃で撃沈しました。`,
      );
    }
    return;
  }
    ...
    for (let dy = -1; dy <= 1; dy++) {
      ...
      if (targetWarship) {
        ...
        if (targetWarship.currentDurability <= 0) {
          ...
          targetWarship.currentAmmo = 0;
          logAction(
            `自爆により軍艦 ${targetWarship.name} (${nx},${ny}) は${damage}のダメージを受け撃沈しました！`,
          );
        } else {
          logAction(
            `自爆により軍艦 ${targetWarship.name} (${nx},${ny}) は${damage}のダメージを受けました。残り耐久: ${targetWarship.currentDurability}`,
          );
        }
      }
      ...
      if (monsterHit) {
        ...
        const monsterName = MONSTER_TYPES[monsterHit.typeId]
          ...
          : "怪獣";
        logAction(
          `自爆により ${monsterName} (${nx},${ny}) は${damage}のダメージを受けました。残り体力: ${monsterHit.hp}`,
        );
        if (monsterHit.hp <= 0) {
          handleMonsterDefeat(
            monsterHit,
            `${monsterName} は自爆に巻き込まれ討伐されました！`,
          );
        }
      }
    }
...
function spawnKingMonsterFromCode() {
  ...
  logAction(
    `(${spawn.x},${spawn.y}) に 怪獣キングガロス (体力: ${hp}) が出現‼`,
  );
}
function initMap() {
...
function updateStatus() {
  ...
  document.getElementById("money").style.visibility = isViewingOtherIsland
    ? "hidden"
    : "visible";
  document.getElementById("food").style.visibility = isViewingOtherIsland
    ? "hidden"
    : "visible";
  document.getElementById("population").style.visibility = isViewingOtherIsland
    ? "hidden"
    : "visible";
  if (trackedFundingFailure) {
...
window.updateConfirmButton = function () {
  ...
  if (action === "exportFood") {
    ...
    document.getElementById("resupplyAmmoAmount").style.display =
      "inline-block";
    ...
    document.getElementById("warshipNicknameInput").style.display =
      "inline-block";
  } else if (action === "convertAchievementToExp") {
    document.getElementById("achievementConvertAmount").style.display =
      "inline-block";
  ...
  for (let y = 0; y < SIZE; y++) {
    ...
    for (let x = 0; x < SIZE; x++) {
      ...
      if (
        ...
        selectedX !== null &&
        selectedY !== null &&
        Math.abs(x - selectedX) <= previewRange &&
        Math.abs(y - selectedY) <= previewRange
      ) {
...
function showTileInfo(x, y) {
  ...
  if (tile.facility) {
    ...
    if (tile.facility === "house" && !isViewingOtherIsland)
      info += ` (人口: ${tile.pop})`; // 他の島では人口表示なし
  ...
  if (warshipAtTile && !isViewingOtherIsland) {
    ...
    const warshipNameDisplay = warshipCapClass
      ? `<span class="${warshipCapClass}">${baseWarshipName}</span>`
      : baseWarshipName;
  ...
  const monsterAtTile = monsters.find((m) => m.x === x && m.y === y);
  if (monsterAtTile) {
    const typeInfo = MONSTER_TYPES[monsterAtTile.typeId] || {
      name: "不明な怪獣",
    };
...
function selectTile(x, y) {
  ...
  if (
    clickedSelectedTile &&
    action &&
    isSessionSettingEnabled("settingConfirmOnSelectedTileClick") &&
...
function logAction(msg, options = {}) {
  ...
  if (options.subtle) {
    ...
    msg.includes("廃墟") ||
    msg.includes("壊滅") ||
    msg.includes("踏み荒らしました") ||
    msg.includes("怪獣が出現") ||
    msg.includes("自爆") ||
    msg.includes("攻撃されました") ||
    msg.includes("砲撃") ||
    msg.includes("破壊されました") ||
    msg.includes("撃沈") ||
    msg.includes("枯渇") ||
    msg.includes("経済危機") ||
    msg.includes("隆起") ||
  ...
  if (
    ...
    isSessionSettingEnabled("settingEnablePlanTracking") &&
    msg.includes("失敗") &&
    msg.includes("資金不足")
  ) {
...
async function generateTouristCode() {
  const simplifiedMap = map.map((row) =>
    row.map((tile) => {
      let touristTerrain = tile.terrain;
      let touristFacility = tile.facility;
      if (
        touristFacility === "gun" ||
        touristFacility === "defenseFacility" ||
        touristFacility === "Monument"
      ) {
...
function saveGame() {
  ...
  encodeManualSaveText(jsonString)
    .then((encodedData) => {
...
async function loadGame() {
  ...
  try {
    ...
    logAction(
      "セーブデータの読み込みに失敗しました。データが破損しているか、形式が不正です。",
    );
    console.error(e);
  }
}
...
function saveMyIslandState() {
  ...
  if (
    window.HakoniwaSupabase &&
    typeof window.HakoniwaSupabase.onLocalStateChanged === "function"
  ) {
    window.HakoniwaSupabase.onLocalStateChanged(
      JSON.parse(JSON.stringify(myIslandState)),
    );
  }
}
...
function loadMyIslandState() {
  if (window.HAKONIWA_SUPABASE_MODE) {
    if (
      window.HakoniwaSupabase &&
      typeof window.HakoniwaSupabase.loadCurrentIslandIntoCore === "function"
    ) {
  ...
  dockedWarships = myIslandState.dockedWarships
    ? JSON.parse(JSON.stringify(myIslandState.dockedWarships))
    : [];
...
async function handleWarshipAttacks() {
  ...
  for (const ship of warships) {
    if (
      ship.currentDurability > 0 &&
      !isWarshipDocked(ship) &&
      ship.currentAmmo > 0 &&
      !(await ensureAndVerifyOwnWarship(ship))
    ) {
  ...
  activeAttackingWarships.forEach((warship) => {
    ...
    logAction(
      `${warship.name} (${warship.homePort}籍) が自動攻撃を開始します。`,
    );
    ...
    for (let i = 0; i < totalAttacks; i++) {
      ...
      while (!validTargetFound && attempts < maxAttempts) {
        targetX =
          warship.x +
          Math.floor(Math.random() * (2 * attackRadius + 1)) -
          attackRadius;
        targetY =
          warship.y +
          Math.floor(Math.random() * (2 * attackRadius + 1)) -
        ...
        if (
          ...
          const isSeaOrWasteWithoutFacility =
            ...
            !warships.find(
              (ship) =>
                ship.x === targetX && ship.y === targetY && ship !== warship,
            ); // Check for other warships
          const targetWarship = warships.find(
            (ship) =>
              ship.x === targetX && ship.y === targetY && ship !== warship,
          );
          const isTargetSameHomeportWarship =
            targetWarship && targetWarship.homePort === warship.homePort;
          const isWreckage =
            targetWarship && targetWarship.currentDurability <= 0;
          if (
            !isSeaOrWasteWithoutFacility &&
            !isTargetSameHomeportWarship &&
            !isWreckage &&
      ...
      if (warship.accuracyImprovement !== 1) {
        protectingDefenseFacility = getProtectingDefenseFacility(
          targetX,
          targetY,
        );
        if (protectingDefenseFacility) {
          ...
          logAction(
            `防衛施設の干渉により、${warship.name} の攻撃精度が低下しました。`,
          );
        }
      }
      ...
      if (Math.random() < hitChance) {
        ...
        if (monsterHit) {
          ...
          if (monsterHit.hp <= 0) {
            handleMonsterDefeat(
              monsterHit,
              `${warship.name} は ${targetType} を討伐し、${expGained} EXPを獲得しました！`,
            );
          } else {
            logAction(
              `${warship.name} は ${targetType} に命中！ (残り体力: ${monsterHit.hp})`,
            );
          }
        } else {
          const otherWarshipAtTarget = warships.find(
            (ship) =>
              ship.x === targetX &&
              ship.y === targetY &&
              ship !== warship &&
              ship.homePort !== warship.homePort,
          );
          if (otherWarshipAtTarget) {
            ...
            if (otherWarshipAtTarget.currentDurability <= 10) {
              logAction(
                `${otherWarshipAtTarget.name}は${warship.name}の砲撃が命中し、黒煙を噴き出しました。 残り耐久: ${otherWarshipAtTarget.currentDurability}`,
              );
            } else if (otherWarshipAtTarget.currentDurability <= 25) {
              logAction(
                `${otherWarshipAtTarget.name}は${warship.name}の砲撃を受けましたが、装甲により損傷は僅かでした。 残り耐久: ${otherWarshipAtTarget.currentDurability}`,
              );
            } else if (otherWarshipAtTarget.currentDurability <= 80) {
              logAction(
                `${otherWarshipAtTarget.name}は${warship.name}の砲撃を受けましたが、ほぼ無傷でした。 残り耐久: ${otherWarshipAtTarget.currentDurability}`,
              );
            } else {
              logAction(
                `${otherWarshipAtTarget.name}は${warship.name}の砲撃を受けましたが、完全に動じないようです。 残り耐久: ${otherWarshipAtTarget.currentDurability}`,
              );
            }
            ...
            if (otherWarshipAtTarget.currentDurability <= 0) {
              ...
              logAction(
                `敵軍艦「${otherWarshipAtTarget.name}」を撃沈しました！`,
              );
            ...
            if (targetTile.MonumentLevel >= 2) {
              ...
              logAction(
                `${warship.name} の攻撃により、石碑のレベルが1低下しました (Lv${targetTile.MonumentLevel})。`,
              );
            } else {
              // レベル1の場合
    ...
    if (attacksPerformed > 0) {
      logAction(
        `${warship.name} は合計 ${attacksPerformed} 回の攻撃を行いました。`,
      );
    }
  });
}
...
window.confirmAction = async function () {
  ...
  const keepOptionSelected =
    document.getElementById("keepOptionSelected").checked;
  if (isViewingOtherIsland) {
    if (
      action === "bombard" ||
      action === "spreadBombard" ||
      ...
      if (
        ...
        window.HakoniwaSupabase &&
        typeof window.HakoniwaSupabase.queueRemoteBombard === "function"
      ) {
        ...
        if (
          window.HakoniwaSupabase.queueRemoteBombard({
            action,
            count,
      ...
      if (count > guns) {
        logAction(
          `砲撃に失敗しました（砲撃数が保有砲台数 (${guns}) を超えています）`,
        );
        return;
      }
      ...
      const encodedAction = btoa(
        encodeURIComponent(JSON.stringify(actionData)),
      );
      document.getElementById("actionForOtherIslandOutput").value =
        encodedAction;
  ...
  if (!action) return;
  applyAutomaticInputValues(action);
  const requiresTileSelection = [
    "buildFarm",
    "buildFactory",
    "buildPort",
    "buildGun",
    "buildDefenseFacility",
    "buildWarship",
    "refuelWarship",
    "resupplyWarshipAmmo",
    "repairWarship",
    "setWarshipNickname",
    "convertAchievementToExp",
    "remodelWarshipWeapon",
    "dispatchWarship",
    "requestWarshipReturn",
    "moveWarshipToDock",
    "returnWarshipFromDock",
    "flatten",
    "landfill",
    "dig",
    "cutForest",
  ...
  if (action === "exportFood") {
    ...
    if (action === "randomBombard") {
      ...
      logAction(
        `(${selectedX},${selectedY}) に ${count}発の${getBombardTypeLabel(action)}を計画しました`,
      );
    }
    ...
    actionQueue.push({
      x: selectedX,
      y: selectedY,
      action: "concentratedFire",
    });
    logAction(`(${selectedX},${selectedY}) に集中砲撃を計画しました`);
  } else if (action === "selfDestructMilitaryFacility") {
    // 名称変更
    if (
      tile &&
      (tile.facility === "gun" || tile.facility === "defenseFacility")
    ) {
    ...
    loadMyIslandState();
    document.getElementById("actionForOtherIslandOutput").value =
      await generateTouristCode();
  } else if (action === "initializeIsland") {
    // 新しいコマンドの処理
    ...
    if (tile.terrain !== "sea" || tile.facility !== null) {
      logAction(
        `(${selectedX},${selectedY}) は軍艦を建造できる海域ではありません。`,
      );
      return;
    }
    // Check if a warship already exists at the selected tile
    const existingWarship = warships.find(
      (ship) => ship.x === selectedX && ship.y === selectedY,
    );
    if (existingWarship) {
      logAction(
        `(${selectedX},${selectedY}) には既に軍艦「${existingWarship.name}」が存在します。`,
      );
      return;
    }
    const name = document.getElementById("warshipName").value.trim();
    const durability = parseInt(
      document.getElementById("warshipDurability").value,
    );
    ...
    if (money < cost) {
      ...
      logAction(
        `軍艦建造に失敗しました（資金不足: ${needed}G が不足しています）`,
      ); // メッセージを修正
      return;
    }
    actionQueue.push({
    ...
    logAction(
      `(${selectedX},${selectedY}) に軍艦「${name}」の建造を計画しました (費用: ${cost}G)`,
    );
  } else if (action === "refuelWarship") {
    // 燃料補給
    const warship = warships.find(
      (ship) => ship.x === selectedX && ship.y === selectedY,
    );
    if (!warship) {
    ...
    logAction(
      `(${selectedX},${selectedY}) の軍艦に燃料 ${amount} を補給する計画を立てました (食料 ${cost} 消費)`,
    );
  } else if (action === "resupplyWarshipAmmo") {
    // 弾薬補給
    const warship = warships.find(
    ...
    actionQueue.push({ x: selectedX, y: selectedY, action, amount });
    logAction(
      `(${selectedX},${selectedY}) の軍艦に弾薬を ${amount} 補給する計画を立てました (資金 ${cost}G 消費)`,
    );
  } else if (action === "repairWarship") {
    if (!targetTileSelected) {
    ...
    if (ship.abnormality === "ammoFire") {
      ...
      if (repairAmount >= 10) {
        ship.abnormality = null;
        logAction(
          `軍艦 ${ship.name} は耐久値を${repairAmount}修理し、弾薬庫の火災は鎮火されました！`,
        );
      }
      ...
      ship.abnormality = null;
      logAction(
        `軍艦 ${ship.name} は耐久値を${repairAmount}修理し、異常状態(${oldAbnormality})が復旧しました！`,
      );
    }
    money -= actualCost;
    ship.currentDurability += actualRepairAmount;
    logAction(
      `${ship.name} を ${actualRepairAmount} 耐久値分修理しました。費用: ${actualCost}G。現在の耐久度: ${ship.currentDurability}/${ship.maxDurability}`,
    );
    renderMap();
    updateStatus();
    saveMyIslandState();
    ...
    const nickname = document
      .getElementById("warshipNicknameInput")
      .value.trim();
    ...
    const amount = parseInt(
      document.getElementById("achievementConvertAmount").value,
    );
    if (isNaN(amount) || amount <= 0) {
    ...
    actionQueue.push({ x: selectedX, y: selectedY, action, amount });
    logAction(
      `軍艦 ${ship.name} に実績Pt ${amount} をEXPへ変換する計画を追加しました。`,
    );
  } else if (action === "remodelWarshipWeapon") {
    const ship = warships.find((s) => s.x === selectedX && s.y === selectedY);
    if (!ship) {
    ...
    const ok = confirm(
      `${ship.name} を${weaponLabel}換装しますか？\nEXP全消費 + 弾薬庫上限1000消費します。`,
    );
    if (!ok) {
  ...
  } else if (action === "dispatchWarship") {
    // 軍艦派遣
    const warship = warships.find(
      (ship) => ship.x === selectedX && ship.y === selectedY,
    );
    ...
    const encodedDispatchAction = btoa(
      encodeURIComponent(JSON.stringify(dispatchData)),
    );
    document.getElementById("actionForOtherIslandOutput").value =
      encodedDispatchAction;
    warship.isDispatched = true; // 派遣状態にする
    warship.currentFuel = 0; // 派遣中は燃料0
    warship.currentAmmo = 0; // 派遣中は弾薬0
    logAction(
      `軍艦「${warship.name}」を「${targetIslandName}」へ派遣する計画を立てました。`,
    );
    logAction(`他島への行動が「他島への行動」欄に出力されました。`);
    ...
    if (!hasValidWarshipNameSignature(warship)) {
      logAction(
        `この軍艦の母港は${warship.homePort}であり、この島ではありません。`,
      );
      return;
    }
    ...
    const encodedReturnRequestAction = btoa(
      encodeURIComponent(JSON.stringify(returnRequestData)),
    );
    document.getElementById("actionForOtherIslandOutput").value =
      encodedReturnRequestAction;

    logAction(
      `軍艦「${warship.name}」の帰還を要請しました。他島への行動が「他島への行動」欄に出力されました。`,
    );
    ...
    const warship = warships.find(
      (ship) =>
        ship.x === selectedX && ship.y === selectedY && !isWarshipDocked(ship),
    );
    if (!warship) {
    ...
    logAction(
      `軍艦「${warship.name}」を船渠へ移動しました（${dockedWarships.length}/5）。`,
    );
    renderDockedWarshipCards();
    renderMap();
    updateStatus();
    ...
    const list = dockedWarships
      .map(
        (ship, index) =>
          `${index + 1}: ${getWarshipDisplayName(ship)} (耐久 ${ship.currentDurability}/${ship.maxDurability}, 主砲 ${ship.mainGun}, 魚雷 ${ship.torpedo})`,
      )
      .join("\n");
    const input = prompt(`海域へ復帰する軍艦番号を入力してください:\n${list}`);
    const dockIndex = parseInt(input, 10) - 1;
    if (
      Number.isNaN(dockIndex) ||
      dockIndex < 0 ||
      dockIndex >= dockedWarships.length
    ) {
    ...
    logAction(
      `軍艦「${warship.name}」を (${selectedX},${selectedY}) の海域へ復帰させました。`,
    );
    renderDockedWarshipCards();
    renderMap();
    updateStatus();
    ...
    if (ship.exp < expCost) {
      logAction(
        `${ship.name} の経験値が不足しています。（${expCost}EXP 必要）`,
      );
      return;
    }
    ...
    do {
      ...
      if (rand < 35) {
        ...
        ship.currentDurability = Math.min(
      ...
      isSessionSettingEnabled("settingEnhanceWarshipToExpLimit") &&
      ship.exp >= expCost
    );
    const resultSummary =
      enhancementResults.length === 1
        ? enhancementResults[0]
        : `${enhancementResults.length}回実行（${enhancementResults.join("、")}）`;
    logAction(
      `${ship.name} を増強しました！ ${resultSummary}を獲得。現在の経験値: ${ship.exp}`,
    );
    renderMap();
    updateStatus();
    saveMyIslandState();
    ...
    logAction(
      `(${selectedX},${selectedY}) を掘削する計画を立てました (発掘率Lv:${factor})`,
    );
  } else {
    actionQueue.push({
      x: selectedX,
      y: selectedY,
      action,
      keepSelected: keepOptionSelected,
    });
...
window.nextTurn = async function () {
  ...
  warships.forEach((warship) => {
    ...
    if (warship.abnormality === "commFailure" && !warship.isDispatched) {
      ...
      logAction(
        `軍艦 ${warship.name} は帰港したため、通信障害が自動復旧しました。`,
      );
    ...
    switch (warship.abnormality) {
      case "fire":
        ...
        logAction(
          `軍艦 ${warship.name} は火災により1ダメージを受けました。残り耐久: ${warship.currentDurability}`,
        );
        // 自動復旧: 25%の確率
        if (Math.random() < 0.25) {
          warship.abnormality = null;
      ...
      case "flooding":
        ...
        registerWarshipDamageTaken(warship, 1);
        logAction(
          `軍艦 ${warship.name} は浸水により1ダメージを受けました。残り耐久: ${warship.currentDurability}`,
        );
        // 自動復旧: 8%の確率
        if (Math.random() < 0.08) {
          warship.abnormality = null;
      ...
      case "ammoFire":
        ...
        logAction(
          `軍艦 ${warship.name} は弾薬庫の発火により3ダメージを受け、弾薬を50消費しました。残り耐久: ${warship.currentDurability}、残り弾薬: ${warship.currentAmmo}`,
        );
        if (warship.currentAmmo === 0) {
          warship.abnormality = null;
          logAction(
            `軍艦 ${warship.name} は弾薬が尽きたため、弾薬庫の発火が収まりました。`,
          );
        }
        break;
      case "commFailure":
  ...
  if (otherIslandActionCode) {
    if (otherIslandActionCode === "KING_MONSTER") {
      ...
      try {
        ...
        if (
          (incomingAction.type === "bombard" ||
            incomingAction.type === "spreadBombard" ||
            incomingAction.type === "ppBombard" ||
            incomingAction.type === "randomBombard") &&
          incomingAction.count !== undefined &&
          (incomingAction.type === "randomBombard" ||
            (incomingAction.x !== undefined && incomingAction.y !== undefined))
        ) {
          ...
          logAction(
            `他島から ${count}発の${getBombardTypeLabel(type)}を受けました！`,
          );
          for (let i = 0; i < count; i++) {
            ...
            if (type === "randomBombard") {
              ...
              if (errorRange > 0) {
                dx =
                  Math.floor(Math.random() * (2 * errorRange + 1)) - errorRange;
                dy =
                  Math.floor(Math.random() * (2 * errorRange + 1)) - errorRange;
              }
              tx = x + dx;
            ...
            if (tx >= 0 && ty >= 0 && tx < SIZE && ty < SIZE) {
              ...
              if (protectingFacility) {
                // 防衛施設があった場合
                if (type === "ppBombard" || type === "randomBombard") {
                  // PP弾・ランダム弾だった場合
                  ...
                  logAction(
                    `(${tx},${ty}) を守っていた防衛施設が${getBombardTypeLabel(type)}により破壊されました！`,
                  );
              ...
              if (target.terrain === "sea") {
                if (target.facility === "port") {
                  ...
                  logAction(
                    `他島からの砲撃により (${tx},${ty}) の港が破壊されました`,
                  );
                } else {
                  const targetWarship = warships.find(
                    (ship) => ship.x === tx && ship.y === ty,
                  );
                  if (targetWarship) {
                    ...
                    if (targetWarship.currentDurability <= 0) {
                      ...
                      warships = warships.filter(
                        (ship) => ship !== targetWarship,
                      );
                      logAction(
              ...
              if (monsterHit) {
                ...
                const monsterName = MONSTER_TYPES[monsterHit.typeId]
                  ...
                  : "怪獣";
                logAction(
                  `他島からの砲撃が ${monsterName} に命中！ (残り体力: ${monsterHit.hp})`,
                );
                if (monsterHit.hp <= 0) {
                  handleMonsterDefeat(
                    monsterHit,
                    `${monsterName} は討伐されました！`,
                  );
                }
              }
            } else {
        ...
        } else if (incomingAction.type === "warshipDispatch") {
          // 軍艦派遣の受信処理
          ...
          if (possibleSeaTiles.length > 0) {
            const randomIndex = Math.floor(
              Math.random() * possibleSeaTiles.length,
            );
            ...
            logAction(
              `「${sourceIslandName}」から軍艦「${newWarship.name}」が派遣されました！ (${x},${y}) に到着。`,
            );
          } else {
            logAction(
              `「${sourceIslandName}」から派遣された軍艦「${newWarship.name}」は配置できる海域がなく、帰還しました。`,
            );
          }
        } else if (incomingAction.type === "warshipReturnRequest") {
          const { homePort, name } = incomingAction;
          const warshipToReturn = warships.find(
            (ship) =>
          ...
          if (warshipToReturn) {
            ...
            const encodedReturnConfirmationAction = btoa(
              encodeURIComponent(JSON.stringify(returnConfirmationData)),
            );
            document.getElementById("actionForOtherIslandOutput").value =
              encodedReturnConfirmationAction;

            // 軍艦をこの島から削除
            warships = warships.filter((ship) => ship !== warshipToReturn);
            logAction(
              `軍艦「${warshipToReturn.name}」(${warshipToReturn.homePort}籍) の帰還要請を受け、返送コードを出力し、自島から削除しました。`,
            );
          } else {
            logAction(
              `帰還要請された軍艦「${name}」(${homePort}籍) は、この島には存在しませんでした。`,
            );
          }
        } else if (incomingAction.type === "warshipReturnConfirmation") {
          // 軍艦帰還確認の受信処理 (母港側)
          ...
          if (existingWarship) {
            ...
            logAction(
              `軍艦「${existingWarship.name}」が母港「${originalHomePort}」に帰還しました！情報が更新され、派遣状態が解除されました。`,
            );
          } else {
            logAction(
              `帰還した軍艦「${returnedWarshipData.name}」は、この島で対応する派遣中の軍艦が見つかりませんでした。`,
            );
          }
        }
      } catch (e) {
  ...
  for (let i = 0; i < ACTIONS_PER_TURN; i++) {
    ...
    if (x !== null && y !== null) {
      ...
      const newWarship = {
        ...
        maxDurability: Math.min(
          WARSHIP_CAPS.maxDurability,
          parseInt(document.getElementById("warshipDurability").value),
        ),
        ...
        mainGun: Math.min(
          WARSHIP_CAPS.mainGun,
          parseInt(document.getElementById("warshipMainGun").value),
        ),
        torpedo: parseInt(document.getElementById("warshipTorpedo").value), // 魚雷には上限なし
        antiAir: Math.min(
          WARSHIP_CAPS.antiAir,
          parseInt(document.getElementById("warshipAntiAir").value),
        ),
        maxAmmo: Math.min(
          WARSHIP_CAPS.maxAmmo,
          parseInt(document.getElementById("warshipAmmo").value),
        ),
        currentAmmo: Math.min(
          WARSHIP_CAPS.maxAmmo,
          parseInt(document.getElementById("warshipAmmo").value),
        ),
        reconnaissance: parseInt(document.getElementById("warshipRecon").value),
        accuracyImprovement: parseInt(
          document.getElementById("warshipAccuracy").value,
        ),
    ...
    if (action === "delayAction") {
      ...
      const targetX = x;
      const targetY = y;
      const availableWarships = warships.filter(
        (ship) =>
          ship.currentDurability > 0 &&
          !ship.isDispatched &&
          ship.currentAmmo > 0 &&
      ...
      if (availableWarships.length === 0) {
        logAction(
          `(${targetX},${targetY}) への集中砲撃は実行されませんでした（攻撃可能な軍艦がありません）。`,
        );
      ...
      const canParticipate = (ship) => {
        const dist = Math.max(
          Math.abs(ship.x - targetX),
          Math.abs(ship.y - targetY),
        );
      ...
      warship.currentAmmo = 0;
      logAction(
        `軍艦「${warship.name}」の緊急帰還を実行しました（${cost}G消費）。`,
      );
      updateStatus();
      saveMyIslandState();
      ...
      if (tile && tile.terrain === "plain" && money >= 100) {
        ...
        checkAndCompleteMission(
          ...
          0,
          250,
          () => map.flat().some((t) => t.facility === "farm"),
          "農場が1つでも建設されている",
        );
      } else
        logAction(
          `(${x},${y}) の農場建設は失敗しました（条件不適合または資金不足）`,
        );
      ...
      if (tile && tile.terrain === "plain" && money >= 100) {
        ...
        checkAndCompleteMission(
          ...
          0,
          500,
          () => map.flat().some((t) => t.facility === "factory"),
          "工場が1つでも建設されている",
        );
      } else
        logAction(
          `(${x},${y}) の工場建設は失敗しました（条件不適合または資金不足）`,
        );
      ...
      if (tile && tile.terrain === "plain" && money >= 1200) {
        ...
        logAction(
          `(${x},${y}) の砲台建設は失敗しました（条件不適合または資金不足）`,
        );
    } else if (action === "buildDefenseFacility") {
      // 防衛施設建設
      if (tile && tile.terrain === "plain" && money >= 5000) {
        ...
        checkAndCompleteMission(
          ...
          0,
          1000,
          () => map.flat().some((t) => t.facility === "defenseFacility"),
          "防衛施設が1つでも建設されている",
        );
      } else
        logAction(
          `(${x},${y}) の防衛施設建設は失敗しました（条件不適合または資金不足）`,
        );
    } else if (action === "landfill") {
      const warshipOnTile = warships.find(
        (ship) => ship.x === x && ship.y === y && ship.currentDurability > 0,
      );
      if (warshipOnTile) {
        ...
        logAction(
          `(${x},${y}) の埋め立ては失敗しました（海ではありませんまたは資金不足）`,
        );
      ...
      if (tile && tile.terrain === "sea" && !tile.facility && money >= 3000) {
        ...
        if (adjacentLand) {
          ...
          logAction(
            `(${x},${y}) の港建設は失敗しました（陸地に隣接していませんまたは資金不足）`,
          );
      } else
        logAction(
          `(${x},${y}) の港建設は失敗しました（条件不適合または資金不足）`,
        );
    } else if (action === "dig") {
      const tile = map[y][x];
      ...
      if (money < cost) {
        ...
        if (tile.terrain !== "sea") {
          ...
          if (tile.facility === "oilRig") {
            logAction(
              `(${x},${y}) にはすでに油田が存在するため、発掘は失敗しました。 (コスト: ${cost}G)`,
            );
          } else {
            // 発掘率の計算
            // oilFactor=1 の場合: 2%
            ...
            if (Math.random() < drillProbability) {
              ...
              logAction(
                `(${x},${y}) を海に掘削した結果、油田を発見しました！ (コスト: ${cost}G, 確率: ${Math.round(drillProbability * 100)}%)`,
              );
            } else {
              logAction(
                `(${x},${y}) を掘削しましたが、油田は見つかりませんでした。 (コスト: ${cost}G, 確率: ${Math.round(drillProbability * 100)}%)`,
              );
            }
          }
        }
      ...
      if (usableGuns > 0) {
        ...
        for (let i = 0; i < usableGuns; i++) {
          ...
          if (tx >= 0 && ty >= 0 && tx < SIZE && ty < SIZE) {
            ...
            if (protectingFacility) {
              // 防衛施設があった場合
              if (action === "ppBombard" || action === "randomBombard") {
                // PP弾・ランダム弾だった場合
                protectingFacility.facility = null; // 防衛施設を破壊
                protectingFacility.terrain = "waste"; // 防衛施設の場所を荒地にする
                protectingFacility.enhanced = false; // 強化状態もリセット
                logAction(
                  `(${tx},${ty}) を守っていた防衛施設が${getBombardTypeLabel(action)}により破壊されました！`,
                );
                // その後、貫通弾の効果を適用（既存の攻撃ロジックに流れる）
              } else {
                // 貫通弾でなければ防衛施設が守る
            ...
            if (monsterHit) {
              ...
              const monsterName = MONSTER_TYPES[monsterHit.typeId]
                ...
                : "怪獣";
              logAction(
                `砲撃が ${monsterName} に命中！ (残り体力: ${monsterHit.hp})`,
              );
              if (monsterHit.hp <= 0) {
                handleMonsterDefeat(
                  monsterHit,
                  `${monsterName} が砲撃により討伐されました‼`,
                );
              }
              hits++; // 命中カウント
              continue; // タイル破壊はスキップ
              ...
              if (target.facility === "port") {
                ...
                if (targetWarship) {
                  ...
                  if (targetWarship.currentDurability <= 0) {
                    ...
                    logAction(
                      `砲撃により軍艦「${targetWarship.name}」が撃沈されました！`,
                    );
                  } else {
                    logAction(
                      `砲撃が軍艦「${targetWarship.name}」に着弾しました！ (残り耐久: ${targetWarship.currentDurability})`,
                    );
                  }
                } else {
                  renderMap();
    ...
    } else if (action === "enhanceFacility") {
      // 設備強化
      if (
        tile &&
        (tile.facility === "farm" ||
      ...
      if (warship && !warship.isDispatched) {
        ...
        if (food >= cost) {
          const actualRefuelAmount = Math.min(
            amount,
            warship.maxFuel - warship.currentFuel,
          );
          warship.currentFuel += actualRefuelAmount;
          food -= cost;
          logAction(
            `軍艦「${warship.name}」に燃料を ${actualRefuelAmount} 補給しました。`,
          );
        } else {
          logAction(
            `軍艦「${warship.name}」への燃料補給に失敗しました（食料不足）。`,
          );
        }
      } else {
      ...
      if (warship && !warship.isDispatched) {
        ...
        if (money >= cost) {
          const actualResupplyAmount = Math.min(
            amount,
            warship.maxAmmo - warship.currentAmmo,
          );
          warship.currentAmmo += actualResupplyAmount;
          money -= cost;
          logAction(
            `軍艦「${warship.name}」に弾薬を ${actualResupplyAmount} 補給しました。`,
          );
        } else {
          logAction(
            `軍艦「${warship.name}」への弾薬補給に失敗しました（資金不足）。`,
          );
        }
      } else {
        logAction(`(${x},${y}) には軍艦が存在しないか、派遣中でした。`);
      ...
      if (warship && warship.homePort === islandName && money >= 100000) {
        ...
        money -= 100000;
        logAction(
          `軍艦「${warship.name}」に二つ名「${task.nickname}」を設定しました。`,
        );
      } else {
      ...
      if (
        ...
        logAction(
          `軍艦「${warship.name}」へ ${amount * 100} EXP を付与しました。`,
        );
      } else {
      ...
      if (warship && warship.maxAmmo >= 1000 && warship.exp > 0) {
        ...
        logAction(
          `軍艦「${warship.name}」の武器換装を実行しました（${task.weaponType === "torpedo" ? "魚雷" : "主砲"}+1）。`,
        );
      } else {
  ...
  for (const warship of warships) {
    if (
      !warship.isDispatched &&
      !isWarshipDocked(warship) &&
      warship.currentFuel >= 1
    ) {
      // 派遣中でない軍艦のみ移動
      ...
      for (let dx = -1; dx <= 1; dx++) {
        for (let dy = -1; dy <= 1; dy++) {
          ...
          if (nx >= 0 && ny >= 0 && nx < SIZE && ny < SIZE) {
            ...
            const isSeaWithoutFacilityOrWarship =
              targetTile.terrain === "sea" &&
              targetTile.facility === null &&
              !warships.some(
                (otherShip) =>
                  otherShip !== warship &&
      ...
      if (possibleMoves.length > 0) {
        ...
        logAction(
          `軍艦「${warship.name}」は (${warship.x},${warship.y}) から (${newPos.x},${newPos.y}) へ移動しました。`,
        );
        warship.x = newPos.x;
        warship.y = newPos.y;
      } else {
        logAction(
          `軍艦「${warship.name}」は移動できませんでした（移動可能なマスがない）。`,
        );
      }
    }
  }
  ...
  if (turn > 0 && turn % 50 === 0) {
    ...
    if (monumentLevel > 0) {
      ...
      warships.forEach((ship) => {
        ...
        // 3. 沈没していない
        if (
          hasValidWarshipNameSignature(ship) &&
          ship.exp !== "NaN" &&
          ship.currentDurability > 0
        ) {
      ...
      if (increasedCount > 0) {
        logAction(
          `[石碑効果] ${islandName}籍の軍艦 ${increasedCount}隻に経験値 ${expIncrease} を付与しました！ (Lv${monumentLevel}効果)`,
        );
      }
    }
  }
  ...
  if (volcanoTurns > 0) {
    ...
    if (volcanoTurns === 0) {
      logAction(
        `噴火による降灰が終息しました。農場と人口増加が正常に戻ります。`,
      );
    } else {
  ...
  if (hasKingMonster) {
    ...
    if (volunteerMoney > 0) {
      ...
      logAction(
        `怪獣キングガロスの影響で義勇金 ${volunteerMoney}G が支給されました。`,
      );
    }
    if (economicCrisisTurns > 0) {
      money += frozenMoney;
  ...
  if (
    ...
    if (Math.random() < totalProbability) {
      ...
      logAction(
        `経済危機が発生しました！ ${economicCrisisTurns}ターンの間、資金の${Math.round(frozenPercentage * 100)}% (${frozenMoney}G)が凍結されます！`,
      );
    }
  }
  // 経済危機の持続処理
    ...
    if (economicCrisisTurns === 0) {
      logAction(
        `経済危機が終息しました。凍結されていた ${frozenMoney}G が使用可能になります。`,
      );
      money += frozenMoney;
      frozenMoney = 0;
    } else {
  ...
  if (!hasMountain && Math.random() < 0.08) {
    // 8%
    ...
    for (let dx = -1; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        ...
        if (nx >= 0 && ny >= 0 && nx < SIZE && ny < SIZE) {
          ...
          // 対象が軍艦の場合 (追加)
          const targetWarship = warships.find(
            (ship) => ship.x === nx && ship.y === ny,
          );
          if (targetWarship) {
            warships = warships.filter((ship) => ship !== targetWarship);
            logAction(
              `隆起に巻き込まれ、軍艦「${targetWarship.name}」が破壊されました！`,
            );
          }
          if (tile.facility === "house") {
            population -= tile.pop;
  ...
  if (
    ...
    volcanoTurns = 12;
    logAction(
      `火山が噴火しました！ ${volcanoTurns}ターンの間、降灰により農場の生産と人口増加が停止します！`,
    );
  }
  // 台風イベント
  ...
  if (Math.random() < 0.018) {
    ...
    if (targetWarship) {
      if (targetWarship.isDispatched) {
        logAction(
          `派遣中の軍艦「${targetWarship.name}」への隕石衝突は無効でした。`,
        );
      } else {
  ...
  if (food < 0) {
    ...
    if (Math.random() < destroyChance) {
      ...
      if (allTargets.length > 0) {
        ...
        logAction(
          `(${x},${y})の${label}で食料不足により住民が殺到、崩壊しました。`,
        );
      }
    }
  }
  ...
  if (spawnCandidates.length > 0) {
    ...
    for (const typeId in MONSTER_TYPES) {
      ...
      if (type.condition(population, turn) && Math.random() < 0.01) {
        ...
        // 5. 体力(HP)の決定
        const hp =
          Math.floor(Math.random() * (type.maxHP - type.minHP + 1)) +
          type.minHP;
        ...
        logAction(
          `(${spawn.x},${spawn.y}) に ${type.name} (体力: ${hp}) が出現‼`,
        );
  ...
  for (const monster of monstersThisTurn) {
    ...
    if (monsterType.moneyPerTurn && monsterType.moneyPerTurn > 0) {
      ...
      logAction(
        `${monsterType.name} が${monsterType.moneyPerTurn}Gを生産しました。`,
      );
    }
    ...
    for (let i = 0; i < moveCount; i++) {
      ...
      if (possible.length > 0) {
        ...
        if (protectingFacility) {
          ...
          logAction(
            `${monsterType.name}によって (${nx},${ny}) を守っていた防衛施設が破壊されました！`,
          );
        }
    ...
    // 2. 特殊能力の処理
    if (monsterType.ability === "destroyArea" && Math.random() < 0.1) {
      // ヴォルカガロス (10%)
    ...
    } else if (monsterType.ability === "landfillSea" && Math.random() < 0.15) {
      // テラガロス (15%)
      // 荒地にする海（軍艦含む）を探す
      const seaTiles = [];
      for (let y = 0; y < SIZE; y++) {
      ...
      if (seaTiles.length > 0) {
        ...
        if (
          targetWarship &&
          targetWarship.currentDurability > 0 &&
          !targetWarship.isDispatched
        ) {
          logAction(
            `${monsterType.name} が (${x},${y}) の海を埋め立てようとし、軍艦「${targetWarship.name}」に10ダメージ！`,
          );
          targetWarship.currentDurability -= 10;
          checkAbnormalityOnDamage(targetWarship, 10);
          if (targetWarship.currentDurability <= 0) {
          ...
          logAction(
            `${monsterType.name} が (${x},${y}) の海を荒地に変えました！`,
          );
          map[y][x].terrain = "waste";
          map[y][x].facility = null;
          map[y][x].enhanced = false;
      ...
      if (landTiles.length > 0) {
        ...
        const facilityName = tile.facility
          ...
          : tile.terrain;
        logAction(
          `${monsterType.name} が (${x},${y}) の${facilityName}を海に変えました！`,
        );
        if (tile.facility === "house") {
          population -= tile.pop;
          if (population < 0) population = 0;
      ...
      if (kingAction === 1) {
        ...
        for (let yy = 0; yy < SIZE; yy++) {
          for (let xx = 0; xx < SIZE; xx++) {
            if (
              map[yy][xx].facility === "gun" ||
              map[yy][xx].facility === "defenseFacility"
            ) {
        ...
        if (militaryTiles.length > 0) {
          const first =
            militaryTiles[Math.floor(Math.random() * militaryTiles.length)];
          const queue = [first];
          ...
          while (queue.length > 0) {
            ...
            for (let dx = -1; dx <= 1; dx++) {
              for (let dy = -1; dy <= 1; dy++) {
                ...
                if (nx < 0 || ny < 0 || nx >= SIZE || ny >= SIZE) continue;
                if (
                  map[ny][nx].facility === "gun" ||
                  map[ny][nx].facility === "defenseFacility"
                ) {
                  chainedTargets.push({ x: nx, y: ny });
                }
              }
            }
            triggerMilitaryFacilitySelfDestruct(
              current.x,
              current.y,
              `${monsterType.name}の能力`,
            );
            queue.push(...chainedTargets);
          }
        } else {
          logAction(
            `${monsterType.name} は軍事施設自爆を試みましたが、対象がありませんでした。`,
          );
        }
        ...
        while (nx >= 0 && ny >= 0 && nx < SIZE && ny < SIZE) {
          applyKingMonsterTileDestruction(
            nx,
            ny,
            `${monsterType.name}が直進破壊: (${nx},${ny})`,
          );
          monster.x = nx;
          monster.y = ny;
          nx += dir.dx;
        ...
        for (let s = 0; s < steps; s++) {
          ...
          applyKingMonsterTileDestruction(
            nx,
            ny,
            `${monsterType.name}が移動破壊: (${nx},${ny})`,
          );
          monster.x = nx;
          monster.y = ny;
        }
        logAction(
          `${monsterType.name} はランダムに ${steps} マス移動しました。`,
        );
      } else if (kingAction === 5) {
        const adjacentWarships = warships.filter(
          (ship) =>
            ship.currentDurability > 0 &&
            !ship.isDispatched &&
            Math.max(
              Math.abs(ship.x - monster.x),
              Math.abs(ship.y - monster.y),
            ) === 1,
        );
        if (adjacentWarships.length > 0) {
          ...
          logAction(
            `${monsterType.name}の${useRoar ? "咆哮" : "巨大な拳"}！軍艦 ${target.name} に ${damage} ダメージ。`,
          );
          if (target.currentDurability <= 0) {
            ...
            logAction(
              `軍艦 ${target.name} は${monsterType.name}の攻撃で撃沈しました。`,
            );
          }
        } else {
          logAction(
  ...
  const activeWarshipCount = warships.filter(
    (ship) => ship.currentDurability > 0 && !isWarshipDocked(ship),
  ).length;
  const facilityMaintenance =
    gunCount * 600 +
    enhancedGunCount * 600 +
    defenseFacilityCount * 2000 +
    portCount * 2500;
...
function earthquakeEffect(excludedCoords = []) {
  ...
  if (landDestroyed.length > 0) {
    logAction(
      `地震により ${landDestroyed.length} 個の陸地タイルが荒地になりました。`,
    );
  } else {
  ...
  for (let y = 0; y < SIZE; y++) {
    for (let x = 0; x < SIZE; x++) {
      ...
      if (tile.facility === "oilRig" || tile.facility === "port") {
        ...
        logAction(
          `津波により (${x},${y}) の${tile.facility === "oilRig" ? "油田" : "港"}が破壊され海になりました。`,
        );
        tile.terrain = "sea";
        tile.facility = null;
        tile.pop = 0;
        tile.enhanced = false;
        totalTilesAffected++;
      } else if (isAdjacentToSea) {
        // 海に隣接する陸地タイルに対するランダム破壊
        const r = Math.random();
        if (r < 0.3) {
          // 30%の確率で荒地
          if (tile.facility === "house") {
            ...
            logAction(
              `津波により (${x},${y})の${tile.facility || tile.terrain}が荒地になりました。`,
            );
          }
          tile.terrain = "waste";
          tile.facility = null;
  ...
  warships.forEach((ship) => {
    ...
    if (
      ...
      logAction(
        `津波により軍艦 ${ship.name} が ${damage} ダメージを受けました。`,
      );
      if (ship.currentDurability <= 0) {
        ship.currentDurability = 0;
  ...
  if (totalTilesAffected > 0 || portsDestroyed > 0 || oilRigsDestroyed > 0) {
    logAction(
      `津波により沿岸部 ${totalTilesAffected} マスに被害が発生しました。（港 ${portsDestroyed}、油田 ${oilRigsDestroyed} を含む）`,
    );
  } else {
...
function addBulkPlans(action, targetTerrain, targetFacility) {
  ...
  let addedCount = 0;
  const keepOption =
    document.getElementById("keepOptionSelected")?.checked || false;
  // マップ全体(16x16)をループ
  for (let y = 0; y < SIZE; y++) {
    for (let x = 0; x < SIZE; x++) {
      // キューが上限に達していたら終了
      if (actionQueue.length >= MAX_QUEUE_SIZE) {
        logAction(
          "計画キューが上限（20件）に達したため、一括登録を終了します。",
        );
...
window.saveGame = async function () {
  ...
  document.getElementById("saveLoadData").value = encodedData;
  logAction(
    "ゲームがMySave形式（gzip圧縮テキスト）で出力されました。テキストエリアからコピーしてください。",
  );
...
window.loadGame = async function () {
  ...
  if (!encodedData) {
    logAction(
      "ロードするデータがありません。テキストエリアにデータを貼り付けてください。",
    );
    return;
  }
  ...
  try {
    ...
    monsters = gameState.monsters
      ? JSON.parse(JSON.stringify(gameState.monsters))
      : [];
    ...
    logAction(
      "セーブデータの読み込みに失敗しました。データが破損しているか、形式が正しくありません。",
    );
    console.error(e);
  }
};
...
window.HakoniwaCore = {
  ...
  setOtherIslandActionCode: (code) => {
    document.getElementById("otherIslandActionInput").value = code || "";
  },
  setViewingOtherIsland: (state, options = {}) =>
    applyHakoniwaSerializableState(state, {
      ...options,
      viewingOtherIsland: true,
    }),
  setOwnIsland: (state) =>
    applyHakoniwaSerializableState(state, { viewingOtherIsland: false }),
};
